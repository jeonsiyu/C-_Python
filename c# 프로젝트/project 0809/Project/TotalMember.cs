...
namespace Project
{
    public partial class TotalMember : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            using (OracleConnection conn = new OracleConnection(connectionString))
            {
                try
                {
                    ...
                    string sql = "SELECT member_id AS \"ID\", member_name AS \"이름\", member_email AS \"이메일\", " +
                                "member_birth AS \"생년월일\", member_phone AS \"연락처\", member_join_date AS \"가입일\" " +
                                "FROM GAME_MEMBER";
        ...
        {
            string member_id = textBox_search.Text.Trim();
            ...
            using (OracleConnection conn = new OracleConnection(connectionString))
            {
                ...
                {
                    conn.Open();
                    string sql = "SELECT member_name AS \"이름\", member_id AS \"ID\", member_email AS \"이메일\", " +
                                "member_birth AS \"생년월일\", member_phone AS \"연락처\", member_join_date AS \"가입일\" " +
                    ...
                    if (!string.IsNullOrEmpty(member_id))
                    {
                        cmd.Parameters.Add(new OracleParameter("SearchText", member_id));
                    }
